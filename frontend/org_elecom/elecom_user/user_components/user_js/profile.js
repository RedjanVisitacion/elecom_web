document.addEventListener("DOMContentLoaded", function () {
  ...
  const saveChangePassword = async () => {
    ...
    if (btnSaveChangePassword) {
      ...
      btnSaveChangePassword.dataset.originalText =
        btnSaveChangePassword.dataset.originalText || btnSaveChangePassword.textContent;
    ...
    try {
      ...
      if (btnSaveChangePassword) {
        ...
        btnSaveChangePassword.textContent =
          btnSaveChangePassword.dataset.originalText || "Save";
      }
    }
  };
  ...
  const saveEditInfo = async () => {
    ...
    if (btnSaveEditInfo) {
      ...
      btnSaveEditInfo.dataset.originalText =
        btnSaveEditInfo.dataset.originalText || btnSaveEditInfo.textContent;
  ...
  const loadProfile = async () => {
    ...
    const yearVal = student.year != null ? String(student.year) : "";
    ...
    try {
      if (fullName) sessionStorage.setItem("elecom_user", fullName);
    } catch (e) {
      // ignore
    }
  ...
  const getCloudinarySignature = async (type) => {
    const url = `${window.location.origin}/api/cloudinary/signature/?type=${encodeURIComponent(type)}`;
    ...
    profilePhotoFile.addEventListener("change", async () => {
      ...
      try {
        if (btnChangeProfilePhoto) {
          ...
          btnChangeProfilePhoto.dataset.originalText =
            btnChangeProfilePhoto.dataset.originalText || btnChangeProfilePhoto.textContent;
          btnChangeProfilePhoto.textContent = "Uploading...";
        }
        ...
        if (btnChangeProfilePhoto) {
          ...
          btnChangeProfilePhoto.textContent =
            btnChangeProfilePhoto.dataset.originalText || "Change photo";
        }
        try {
  ...
  try {
    const cachedPhoto = String(sessionStorage.getItem("elecom_user_photo_url") || "").trim();
    if (cachedPhoto) setMenuAvatar(cachedPhoto);
  } catch (e) {
    // ignore
  }
